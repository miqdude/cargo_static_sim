    ...
    try:
        ...
        while True:
            ...
            if socket_est in socks:
                ...
                est_mass = latest_est["estimated_mass"]
                est_wind_x = latest_est["estimated_wind_x"]
                est_wind_y = latest_est["estimated_wind_y"]
                est_wind_z = latest_est["estimated_wind_z"]

                # harusnya diconvert jadi offset dulu
                payload_mass_guess = max(0.05, est_mass - FRAME_MASS)
            ...
            if latest_telem is not None:
                ...
                target_pos, target_vel, target_acc, target_rpy, target_ang_vel = get_trajectory(time_now, TRAJECTORY_PATH)

                # =========================================================
                # CONTROL ALLOCATION
                # =========================================================
                print(f"estimated offset {est_offset_x} {est_offset_y} estimated mass : {est_mass} estimated wind {est_wind_x} {est_wind_y} {est_wind_z}")
                thrust_commands, torque_commands, wrench = controller.get_follower_commands(
                    ...
                    target_rpy,
                    FRAME_MASS,
                    payload_mass_guess, # payload mass
                )
                
                response = {
                    "thrusts": thrust_commands,
                    "torques": torque_commands,
                    "wrench": wrench,
                    "target_pos": target_pos,
                    "calculated_offset_x": est_offset_x,
                    "calculated_offset_y": est_offset_y,
                }
